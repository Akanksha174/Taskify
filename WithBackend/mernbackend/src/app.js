...
app.get("/profile", async (req, res) => {
    try {
        ...
        // Find user in the database based on the email using the "register" model in the "detail" database
        const user = await detail.register.findOne({ email: userEmail });
        if (!user) {
            return res.status(404).send("User not found");
        }

        // Render profile page with user data only
